...
class Config:
    ...
    CANDIDATE_VIDEO_FRAMES = int(os.getenv("CANDIDATE_VIDEO_FRAMES", "30"))
    ...
    FREQUENCY_CHECKPOINT = MODEL_DIR / os.getenv(
        "FREQUENCY_CHECKPOINT", "frequency_only_best.pth"
    )
    FACE_DETECTOR_DIR = _path_from_env(
        "FACE_DETECTOR_DIR", "artifacts/face_detector"
    )
    FACE_PROTOTXT = FACE_DETECTOR_DIR / "deploy.prototxt"
    FACE_WEIGHTS = FACE_DETECTOR_DIR / "face_detector.caffemodel"
...
def validate_config(config):
    if config["CANDIDATE_VIDEO_FRAMES"] < 1:
        raise ValueError("CANDIDATE_VIDEO_FRAMES must be at least 1.")
    ...
    if config["MAX_VIDEO_FRAMES"] > config["CANDIDATE_VIDEO_FRAMES"]:
        raise ValueError("MAX_VIDEO_FRAMES cannot exceed CANDIDATE_VIDEO_FRAMES.")
