...
def _numpy_checkpoint_safe_globals():
    """
    Return the small NumPy allowlist needed by the project's own checkpoints.

    The Kaggle training notebook stored validation metrics as NumPy scalar
    values alongside model_state. PyTorch 2.6 and newer reject those scalars
    under the restricted weights-only loader unless their exact NumPy types
    ...
    allowed = [scalar_function, np.dtype]

    # NumPy 1.24 and earlier use dtype classes such as
    # type(np.dtype(np.float64)); NumPy 1.25+ exposes equivalent classes under
    # np.dtypes. Registering the runtime classes supports both layouts.
    for value_type in (
        np.float16,
        np.float32,
        np.float64,
        ...
        allowed.append(value_type)
        allowed.append(type(np.dtype(value_type)))

    # Preserve order while removing duplicate dtype classes.
    return list(dict.fromkeys(allowed))


def _trusted_torch_load(path, device):
    """Load a project checkpoint without enabling unrestricted unpickling."""
    safe_globals = getattr(torch.serialization, "safe_globals", None)

    if safe_globals is not None:
        with safe_globals(_numpy_checkpoint_safe_globals()):
            return torch.load(path, map_location=device, weights_only=True)

    # Compatibility with older PyTorch releases. Those releases may accept
    # weights_only but do not provide the safe_globals context manager.
    try:
        return torch.load(path, map_location=device, weights_only=True)
    except TypeError:
...
class ModelBundle:
    ...
    frequency_model: FrequencyOnlyModel
...
def load_model_bundle(config):
    required_paths = [
        ...
        Path(config["FREQUENCY_CHECKPOINT"]),
    ...
    if config["VERIFY_DUAL_SHA256"]:
        ...
        expected_frequency_hash = metadata.get(
            "frequency_only_checkpoint_sha256"
        )
        if not expected_frequency_hash:
            raise ArtifactError(
                "model_metadata.json does not identify the frequency-only "
                "checkpoint."
            )
        frequency_hash = sha256_file(config["FREQUENCY_CHECKPOINT"])
        if frequency_hash.lower() != str(expected_frequency_hash).lower():
            raise ArtifactError(
                "frequency_only_best.pth does not match its expected SHA-256."
            )
    ...
    frequency_model = FrequencyOnlyModel(dropout=0.4).to(device)
    frequency_checkpoint = _trusted_torch_load(
        config["FREQUENCY_CHECKPOINT"], device
    )
    frequency_model.load_state_dict(
        _state_dict_from_checkpoint(
            frequency_checkpoint, config["FREQUENCY_CHECKPOINT"]
        ),
        strict=True,
    )
