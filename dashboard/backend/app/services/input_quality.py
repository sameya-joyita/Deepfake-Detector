...
def assess_input_adequacy(extraction, minimum_usable_frames):
    ...
    return {
        # Notebook protocol: sample up to 30 candidate frames, then
        # uniformly retain at most 20 successful face crops for inference.
        "requested_frames": int(extraction.maximum_model_frames),
        "candidate_frames_requested": int(extraction.requested_frames),
        "total_video_frames": int(extraction.total_video_frames),
        "candidate_frames_sampled": int(
            len(extraction.sampled_frame_indices)
        ),
        ...
        "face_candidates_detected": int(
            extraction.face_candidates_detected
        ),
        "maximum_model_frames": int(extraction.maximum_model_frames),
        "model_frames_discarded": int(extraction.model_frames_discarded),
        "usable_frames": int(usable_frames),
        "face_detection_failures": int(extraction.face_detection_failures),
        "usable_fraction": float(
            usable_frames / extraction.maximum_model_frames
        ),
