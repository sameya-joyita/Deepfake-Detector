"""Locked spatial, frequency-only, dual and frequency-disabled scoring."""
...
class InferenceResult:
    ...
    frame_frequency_only_scores: list
...
class InferenceEngine:
    ...
    def score(self, crops_rgb):
        ...
        self.bundle.frequency_model.eval()
        ...
        start = time.perf_counter()
        with torch.inference_mode():
            frequency_logits = self.bundle.frequency_model(raw)
        _synchronise(self.bundle.device)
        frequency_ms = (time.perf_counter() - start) * 1000
        ...
        frequency_scores = torch.sigmoid(frequency_logits).detach().float().cpu()
        ...
        frequency_video_score = float(frequency_scores.mean())
        ...
        return InferenceResult(
            ...
            frequency_only_score=frequency_video_score,
            ...
            frame_frequency_only_scores=frequency_scores.numpy().tolist(),
            ...
            timings_ms={
                ...
                "frequency_only_forward_batch": float(frequency_ms),
