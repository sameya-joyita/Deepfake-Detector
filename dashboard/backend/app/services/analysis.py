...
class AnalysisService:
    ...
    def health(self):
        return {
            ...
            "models_loaded": ["spatial", "frequency_only", "dual"],
    ...
    def analyse_video(self, video_path, analysis_id, filename, include_gradcam=True):
        ...
        with self.analysis_lock:
            ...
            extraction = self.extractor.extract_video(
                video_path,
                requested_frames=self.config["CANDIDATE_VIDEO_FRAMES"],
                maximum_model_frames=self.config["MAX_VIDEO_FRAMES"],
            )
            preprocessing_ms = (
        ...
        processing = {
            "face_extraction_ms": float(preprocessing_ms),
            ...
            "device": str(self.bundle.device),
            "note": (
                "preprocessing_ms measures video decoding, face detection, "
                "crop resizing and the JPEG round trip. Total pipeline time "
                "also includes input-quality checks, tensor preparation, "
                "model inference, frame-preview encoding and optional "
                "Grad-CAM; it excludes network transfer."
            ),
        }
                ...
                record.update(
                    {
                        ...
                        "frequency_only_score": float(
                            inference.frame_frequency_only_scores[index]
                        ),
